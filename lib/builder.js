...
const { processImages, convertImagesToWebp } = require('./images.js');
...
/**
 * Generate posts list HTML with lazy load support
 */
function generatePostsListHtml(posts, config) {
  ...
  const initialCount = 8;  // Show first 8 posts
  const loadMoreCount = 5; // Load 5 more each time

  let html = '<ul class="posts-list" id="posts-list">';
  sortedPosts.forEach((post, index) => {
    const isHidden = index >= initialCount;
    html += `
      <li class="post-item${isHidden ? ' hidden' : ''}" data-index="${index}">
  ...
  if (sortedPosts.length > initialCount) {
    html += `
    <div class="load-more-container">
      <button id="load-more-btn" class="load-more-btn" data-loaded="${initialCount}" data-total="${sortedPosts.length}" data-increment="${loadMoreCount}">
        Load More
      </button>
    </div>
    <script>
    (function() {
      ...
      btn.addEventListener('click', function() {
        ...
        const newLoaded = Math.min(loaded + increment, total);
        
        // Show hidden posts
        const posts = document.querySelectorAll('#posts-list .post-item.hidden');
        let shown = 0;
...
function buildPost(post, templates, allPosts, config) {
  ...
  return convertImagesToWebp(html);
}
...
async function build(projectDir, options = {}) {
  ...
  if (fs.existsSync(staticDir)) {
    ...
    // Copy other static files (css, js, etc.)
    const staticEntries = fs.readdirSync(staticDir, { withFileTypes: true });
    for (const entry of staticEntries) {
      if (entry.name === 'images') continue; // Skip images, already processed
